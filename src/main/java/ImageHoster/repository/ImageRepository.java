...
--------------------------------------------------------------------------------------------------------------------------------
 *@ 1.0.0.1         22-Dec-2018                  Shankar Ambati              Fix for Image Upload Issue.
 *@ 1.0.0.2         29-Dec-2018                  Shankar Ambati              Bug Fix: Owner of the image can edit/delete the image.
*/
...
public class ImageRepository {
	...
	public Image getImageByTitle(String title, int id) {
		EntityManager em = emf.createEntityManager();
		TypedQuery<Image> typedQuery = null; // Added by Shankar Ambati. Fix for image upload issue.
		try {
			// typedQuery = (TypedQuery<Image>) em.createQuery("SELECT i from Image i where
			// i.title =:title , Image.class)setParameter("title",title);;//Comments added
			// by Shankar Ambati. Fix for image upload issue.
			typedQuery = (TypedQuery<Image>) em.createQuery("SELECT i from Image i where i.title =:title and i.id =:id",
					Image.class); // Added by Shankar Ambati. Fix for image upload issue.
			// Start: Added by Shankar Ambati. Fix for image upload issue.
			typedQuery.setParameter("title", title);
			typedQuery.setParameter("id", id);
			// End: Added by Shankar Ambati. Fix for image upload issue.
	...
	public Image getImage(Integer imageId) {
		...
		Image image = null;
		try {
			typedQuery = em.createQuery("SELECT i from Image i where i.id =:imageId", Image.class)
