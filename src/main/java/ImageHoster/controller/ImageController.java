...
public class ImageController {
	...
	@Autowired
	private CommentService commentService; // Added by Shankar Ambati. Functionality Upgrade: Users can add comments for
											// any image.
	...
	// @RequestMapping("/images/{title}") //Comments added by Shankar Ambati. Fix for
	// image upload issue.
	@RequestMapping("/images/{id}/{title}") // Added by Shankar Ambati. Fix for image upload issue.
	public String showImage(@PathVariable("id") int id, @PathVariable("title") String title, Model model) {
		// Image image = imageService.getImageByTitle(title); //Comments added by Dhruv
		// Sharma. Fix for image upload issue.
		Image image = imageService.getImageByTitle(title, id); // Added by Shankar Ambati. Fix for image upload issue.
		model.addAttribute("image", image);
		model.addAttribute("tags", image.getTags());
		model.addAttribute("comments", commentService.retrieveComments(image));// Added by Shankar Ambati.Functionality
																				// Upgrade: Users can add comments for
																				// any image.
		return "images/image";
	}
	...
	// image
	/*
	 * Start: Added by Shankar Ambati. Bug Fix: Owner of the image can edit the image.
	 * 
	 * @RequestMapping(value = "/editImage") public String
	 * editImage(@RequestParam("imageId") Integer imageId, Model model) { End: Added
	 * by Shankar Ambati. Bug Fix: Owner of the image can edit the image.
	 */

	// Start: Added by Shankar Ambati. Bug Fix: Owner of the image can edit the image.
	@RequestMapping(value = "/editImage")
	public String editImage(@RequestParam("imageId") Integer imageId, Model model, HttpSession session) {
		// End: Added by Shankar Ambati. Bug Fix: Owner of the image can edit the image.
		Image image = imageService.getImage(imageId);

		// Start: Added by Shankar Ambati. Bug Fix: Owner of the image can edit the image.
		String errorPage = "", result = "";
		User user = (User) session.getAttribute("loggeduser");
		Integer imageUserID = imageService.getImage(imageId).getUser().getId(); // User ID corresponding to the
																				// specified image.
	...
	public String editImageSubmit(@RequestParam("file") MultipartFile file, @RequestParam("imageId") Integer imageId,
		...
		// return "redirect:/images/" + updatedImage.getTitle(); //Comments added by
		// Shankar Ambati. Fix for image upload issue.
		return "redirect:/images/" + imageId + "/" + updatedImage.getTitle(); // Added by Shankar Ambati. Fix for image
																				// upload issue.
	}
	...
	/*
	 * Start: Comments added by Shankar Ambati. Bug Fix: Owner of the image can delete
	 * the image.
	 * 
	 * @RequestMapping(value = "/deleteImage", method = RequestMethod.DELETE) public
	 * String deleteImageSubmit(@RequestParam(name = "imageId") Integer imageId) {
	 * End: Comments added by Shankar Ambati. Bug Fix: Owner of the image can delete
	 * the image.
	 */

	// Start: Added by Shankar Ambati. Bug Fix: Owner of the image can delete the
	// image.
	@RequestMapping(value = "/deleteImage", method = RequestMethod.DELETE)
	public String deleteImageSubmit(@RequestParam(name = "imageId") Integer imageId, HttpSession session, Model model) {
		// End: Added by Shankar Ambati. Bug Fix: Owner of the image can delete the image.

		// imageService.deleteImage(imageId); //Comments added by Shankar Ambati. Bug Fix:
		// Owner of the image can delete the image.
		...
		if (imageUserID == loggedUserID) {
			...
			// Re-direct to the same image in case user is not authorized to delete the
			// image.
			result = "redirect:images/" + errorPage + "?" + "deleteError";
		}
		// End: Added by Shankar Ambati. Bug Fix: Owner of the image can delete the image.

		// return "redirect:/images"; //Comments added by Shankar Ambati. Bug Fix: Owner
		// of the image can delete the image.
		return result; // Added by Shankar Ambati. Bug Fix: Owner of the image can delete the image.
	}
